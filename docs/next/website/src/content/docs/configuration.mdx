...
A horizontal trackpad swipe or mouse wheel anywhere on the tab row switches tabs; a wheel takes a few notches per tab. The focused tab's highlight slides toward the neighbor as you swipe and snaps back if you stop short; past either end it wraps to the other end. Set `tab_swipe_extra_rows` under `[ui]` to also accept horizontal swipes in that many pane rows next to the tab row. Vertical scrolling in those rows still goes to the pane.
