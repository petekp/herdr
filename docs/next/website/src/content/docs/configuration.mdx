...
A horizontal trackpad swipe or mouse wheel switches tabs, either on the tab row or over a pane whose program is not using the mouse itself; Claude Code counts, since its fullscreen renderer takes the mouse only to scroll its transcript. A wheel takes a few notches per tab. The focused tab's highlight slides toward the neighbor as you swipe and snaps back if you stop short; past either end it wraps to the other end. The pane area slides with it: the current tab's content moves out and the neighbor's moves in behind the gesture. Set `tab_swipe_transition = "staggered_slide"` under `[ui]` to slide the rows in turn from the top, so the edge between the two tabs runs at a slant, or `"dissolve"` to switch the pane area cell by cell in a fixed grain pattern; the default is `"slide"`. Sideways drift during a vertical scroll is ignored, and so is vertical drift during a swipe; a deliberate change of direction takes effect within a few events.
