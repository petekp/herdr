...
A horizontal trackpad swipe or mouse wheel switches tabs, either on the tab row or over a pane whose program is not using the mouse itself; Claude Code and omp count, since they take the mouse only to scroll vertically. A wheel switches on its second notch, because a single notch cannot yet be told from the start of a swipe. The focused tab's highlight slides toward the neighbor as you swipe and snaps back if you stop short; past either end it wraps to the other end. The pane area slides with it: the current tab's content moves out and the neighbor's moves in behind the gesture. Sideways drift during a vertical scroll is ignored, and so is vertical drift during a swipe; a deliberate change of direction takes effect within a few events.
